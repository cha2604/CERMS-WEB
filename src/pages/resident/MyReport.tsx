...
export default function MyReports() {
  ...
  const [filter, setFilter] = useState<"All" | "Pending" | "On-going" | "Rejected">("All");
  ...
  const filteredReports = reports.filter((r) => {
    ...
    if (filter === "On-going") return r.status === "Ongoing" || r.status === "On-going";
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        <div>
          <h1 className="text-xl font-black text-slate-900">My Reports</h1>
          <p className="text-xs text-slate-500 font-semibold">Active & Pending Concern Submissions</p>
        </div>
        ...
        <div className="flex bg-white p-1 rounded-xl border border-slate-200 text-xs font-bold shadow-sm">
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg transition-all ${
              filter === "All" ? "bg-emerald-800 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100"
            }`}
          >
            All
          </button>
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg transition-all ${
              filter === "Pending" ? "bg-emerald-800 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100"
            }`}
          >
            Pending
          </button>
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg transition-all ${
              filter === "On-going" ? "bg-emerald-800 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100"
            }`}
          >
            On-going
          </button>
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg transition-all ${
              filter === "Rejected" ? "bg-emerald-800 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100"
            }`}
          >
            Rejected
          </button>
            ...
            >
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold border ${getStatusBadgeClass(r.status)}`}>
                    {r.status}
                  </span>
                </div>
